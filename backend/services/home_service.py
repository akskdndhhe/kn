...
#: FINANCE (2026-06) — keputusan yang MENAHAN UANG di Meja Finance: kontrabon yang
#: menunggu verifikasi/persetujuan/keputusan sengketa (pemasok tidak dibayar sampai
#: ada tanda tangan) dan tagihan supplier yang menunggu ACC. Sebelum ini papan
#: "menunggu keputusan" hanya ada di meja sales & gudang, padahal yang paling mahal
#: bila menggantung justru dokumen yang menahan pembayaran.
FINANCE_BOARD_KEYS = ("contra_bon_approve", "contra_bon_verify", "contra_bon_dispute",
                      "vendor_bill")
...
async def _waiting_boards(entity_id: Optional[str],
    ...
    `allowed` = matriks izin peran PEMANGGIL. Dipakai memutuskan baris mana yang boleh
    dapat tombol "Setujui" langsung di papan (2026-06). Wewenang diputuskan SERVER;
    layar tidak pernah menebaknya (dan endpoint tujuannya tetap memeriksa sendiri).
    """
    from services import approval_backlog_service as abl
