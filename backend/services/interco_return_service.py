...
async def on_return_task_executed(transfer: Dict[str, Any], actor: str) -> Dict[str, Any]:
    ...
    rolls = await db.inventory_rolls.find(
        ...
        {"_id": 0, "id": 1, "product_id": 1, "unit_cost": 1, "base_unit_cost": 1,
         "length_remaining": 1, "cost_basis": 1, "roll_no": 1,
         "owner_entity_id": 1}).to_list(10000)
    ...
    for r in rolls:
        ...
        # INV-GL-DRIFT (2026-06, ditutup) — nilai tercatat roll HARUS dibaca dengan
        # rumus yang SAMA dengan rekonsiliasi persediaan (`unit_cost or
        # base_unit_cost`). Dulu di sini hanya `unit_cost` yang dibaca, jadi roll
        # ber-`unit_cost` 0 tetapi `base_unit_cost` masih bernilai (mis. `RTN-00001`
        # Rp 90.000/unit) menghasilkan `carry_out`/`cost_back` = 0: jurnal
        # `interco_return:…:goods_in` DILEWATI padahal barangnya benar-benar masuk
        # gudang penjual — subledger naik, GL 1-1300 tidak → selisih abadi.
        booked = float(r.get("unit_cost") or r.get("base_unit_cost") or 0)
        target = prev if prev > 0 else booked
