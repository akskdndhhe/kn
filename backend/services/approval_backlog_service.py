...
DETAIL_META: Dict[str, Dict[str, List[str]]] = {
    ...
    # 2026-06 (lanjutan ke-4) — TAGIHAN SUPPLIER: papan ini menahan UANG, jadi nominalnya
    # harus terlihat tanpa membuka layar. Field diperiksa dari dokumen `vendor_bills`
    # nyata (`grand_total` · `net_amount`, bukan `total_amount` yang tidak pernah ada).
    "vendor_bill": {
        "amount": ["grand_total", "net_amount"],
        "note": ["supplier_invoice_no", "bill_type"],
        "role": ["required_approval_role"],
    },
    # Kontrabon: nominal yang benar-benar akan dibayar (net setelah retur/potongan).
    "contra_bon_approve": {
        "amount": ["totals.net_payable", "totals.bills_total"],
        "note": ["match_summary.status", "notes"],
    },
    "contra_bon_verify": {
        "amount": ["totals.net_payable", "totals.bills_total"],
        "note": ["match_summary.status", "notes"],
...
#: ── PAPAN BISA DITINDAK (2026-06, lanjutan ke-4) ────────────────────────────────
#: Pintu keputusan yang bisa diselesaikan LANGSUNG dari baris papan. Yang dideklarasikan
#: di sini hanya endpoint yang MEMANG sudah ada di router (lengkap dengan izin & aturan
#: pemisahan tugasnya) — papan tidak boleh punya jalur tulis sendiri, karena itu berarti
#: aturan persetujuan hidup di dua tempat dan cepat atau lambat berbeda.
#:
#: `path` menerima dokumen dan mengembalikan path LENGKAP (server yang menyusunnya, bukan
#: layar — lihat `sales_order` yang butuh id entri `pending_approvals`). Bila `path`
#: mengembalikan kosong, barisnya tidak dapat tombol: lebih baik tidak ada tombol
#: daripada tombol yang menjanjikan sesuatu lalu dijawab 404.
#:
#: SENGAJA TIDAK ADA di sini (dan alasannya):
#:  · `contra_bon_dispute` — keputusan sengketa butuh PILIHAN (terima klaim / tolak /
#:    potong sebagian) + alasan; memeras itu jadi satu tombol "Setujui" akan membuat
#:    orang menekan sesuatu yang artinya tidak jelas.
#:  · `inspection_hold` — pelepasan tahanan bersifat PER BARIS inspeksi (`line_id`) dan
#:    alasannya WAJIB; barisnya papan mewakili dokumen, bukan baris barangnya.
#:  · `payment_variance`, `rnd_sample` — keputusannya memilih di antara beberapa
#:    kemungkinan, bukan "setujui".
def _so_decide_path(doc: Dict[str, Any]) -> str:
    """Path keputusan SO — menunjuk ENTRI `pending_approvals` yang masih menunggu."""
    entry = next((p for p in (doc.get("pending_approvals") or [])
                  if p.get("status") == "pending"), None)
    if not entry or not entry.get("id"):
        return ""
    return f"/sales-orders/{doc['id']}/approvals/{entry['id']}/decide"


ACTION_META: Dict[str, Dict[str, Any]] = {
    "special_order": {
        "label": "Setujui", "method": "POST", "permission": ("order", "approve"),
        "path": lambda d: f"/special-orders/{d['id']}/approve", "note_field": "notes"},
    "sales_order": {
        "label": "Setujui", "method": "POST", "permission": ("order", "approve"),
        # Keputusan menempel pada ENTRI `pending_approvals`, bukan pada dokumennya.
        "path": lambda d: _so_decide_path(d),
        "note_field": "notes", "body": {"decision": "approve"}},
    "price": {
        "label": "Setujui", "method": "POST", "permission": ("price_approval", "approve"),
        "path": lambda d: f"/price-approvals/{d['id']}/approve",
        "note_field": "decision_notes"},
    "transfer": {
        "label": "Setujui", "method": "POST", "permission": ("transfer", "approve"),
        "path": lambda d: f"/transfers/{d['id']}/approve", "note_field": ""},
    "cycle_count": {
        "label": "Setujui", "method": "POST", "permission": ("inventory", "approve_count"),
        "path": lambda d: f"/cycle-count/sessions/{d['id']}/approve",
        "note_field": "reason"},
    "contra_bon_verify": {
        "label": "Verifikasi", "method": "POST", "permission": ("contra_bon", "verify"),
        "path": lambda d: f"/contra-bons/{d['id']}/verify", "note_field": ""},
    "contra_bon_approve": {
        "label": "Setujui", "method": "POST", "permission": ("contra_bon", "approve"),
        "path": lambda d: f"/contra-bons/{d['id']}/approve", "note_field": ""},
    "vendor_bill": {
        "label": "Setujui", "method": "POST", "permission": ("vendor_bill", "approve"),
        "path": lambda d: f"/vendor-bills/{d['id']}/approve", "note_field": ""},
    "interco_return": {
        "label": "Setujui", "method": "POST", "permission": ("interco", "approve"),
        "path": lambda d: f"/interco/returns/{d['id']}/approve", "note_field": "note"},
}
...
def _row_action(key: str, doc: Dict[str, Any],
    ...
    TIDAK diberikan: layar tidak boleh menebak wewenang orang.
    """
    meta = ACTION_META.get(key)
    if not meta or allowed is None:
        return None
    module, action = meta["permission"]
    perms = allowed.get(module) or []
    if action not in perms and "*" not in perms:
        return None
    try:
        path = meta["path"](doc)
    except Exception:  # noqa: BLE001 — dokumen aneh tak boleh menjatuhkan papan
        path = ""
    if not path:
        return None
